...
class Event {
  ...
  get endTime() {
    return this.excelEndTime
      ...
      // endOf('day') produces .995 seconds,
      // but Google calendar does not support sub-second time granularity,
      // so we round down to startOf('second')
      : this.date.endOf('day').startOf('second')
  }
  ...
  simulateAGoogleCalendarEvent(calendar, peopleMapper) {
    ...
    return new GSuiteCalendarEvent({
      properties: {
        ...
        reminders: {
          useDefault: false,
          overrides: [], // GSuite reminders are only sent to the event owner, not attendees :-/
        },
      },
      calendar,
    })
